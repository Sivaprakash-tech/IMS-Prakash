...
async def _process_one(sig: SignalIn) -> None:
    ...
    mongo_id = await retry_async(mongo.insert_signal, raw_doc)
    ...
    if won:
        # ---------- 3a. Create work item (WITH RETRY) ----------
        work_item_id = await retry_async(
            postgres_repo.create_work_item,
            component_id=sig.component_id,
        ...
        # ---------- Redis live feed (WITH RETRY) ----------
        await retry_async(
            redis_cache.push_live_feed,
            work_item_id,
            sig.severity.value,
            sig.component_id,
        )
        ...
        if existing is None:
            ...
            await retry_async(
                postgres_repo.increment_signal_count,
                work_item_id,
            )
        else:
            work_item_id = None
    ...
    if work_item_id is not None:
        ...
        try:
            ...
            log.warning("mongo.backfill.failed", error=str(e))

    # ---------- 5. Metrics ----------
    await retry_async(
        postgres_repo.upsert_metric_bucket,
...
async def _worker_loop(worker_id: int) -> None:
    ...
    while not _shutdown.is_set():
        ...
        try:
            ...
            log.exception("worker.process.error", error=str(e))
        finally:
